defmodule Mocked do
  defmacro __using__(_) do
    quote do
      ...
      def handle_call(
            {:call_count, func, args},
            _from,
            %{call_count: call_count, interceptors: interceptors} = state
          ) do
      ...
      def handle_call(
            {:increment_call_count, func, args},
            _from,
            %{call_count: call_count, interceptors: interceptors} = state
          ) do
        {func, args} = get_matching_args([], interceptors, func, args) |> elem(0)
      ...
      def handle_call(
            {:set_interceptor, func, args, intercept_func},
            _from,
      ...
      defp get_interceptors(interceptors, func, args) do
        found =
          Map.get(interceptors, {func, args}, [])
